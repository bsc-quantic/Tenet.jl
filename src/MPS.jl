...
"""
function compression(ψ::AbstractMPS, M)

    ϕ=copy(ψ')
    L=my_length(ψ)
    
    replace!(ψ, inds(ψ; at=Site(L)) => :a)
    replace!(ϕ, inds(ϕ; at=Site(L)') => :b)
    
    tn= merge(TensorNetwork(ψ), TensorNetwork(ϕ))
    ρ=contract(tn; path=einexpr(tn; optimizer=Exhaustive()))
    rho_inds=inds(ρ)
    left_inds=rho_inds[1]
    right_inds=rho_inds[2]
    u,s,v,err = my_truncate_eig(ρ, [left_inds],[right_inds], :bond, M)
...
function my_truncate_eig(ρ ::Tensor, left_inds,  right_inds, bond, maxdim=nothing)
    ...
    if maxdim !== nothing
        ...
        u=view(u, bond => 1:maxdim)
        s=view(s, bond => 1:maxdim)
        v=view(v, bond => 1:maxdim)
    end

    return u,s,v, error
end

function my_length(ψ ::MPS)
    inds_out=sort(inds(ψ; set=:open))
    L=length(inds_out)
    return L
end
