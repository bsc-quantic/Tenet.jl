...
function canonize!(ψ::AbstractMPO; normalize=false)
    ...
    for i in nsites(ψ; set=:outputs):-1:2
    ...
    for i in 1:(nsites(ψ; set=:outputs) - 1)
    ...
    for i in 2:nsites(ψ; set=:outputs) # tensors at i in "A" form, need to contract (Λᵢ)⁻¹ with A to get Γᵢ
