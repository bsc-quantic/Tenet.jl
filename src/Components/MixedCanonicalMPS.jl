...
mutable struct MixedCanonicalMatrixProductState <: AbstractMPS
    ...
    unsafe::Ref{Union{Nothing,Tangles.UnsafeScope}}
end

const MixedCanonicalMPS = MixedCanonicalMatrixProductState

function Base.copy(tn::MixedCanonicalMPS)
    unsafe = Ref{Union{Nothing,Tangles.UnsafeScope}}(tn.unsafe[])
...
function MixedCanonicalMPS(_form::MixedCanonical, arrays; order=defaultorder(MixedCanonicalMPS), unsafe=nothing) # , check=true)
    ...
    return MixedCanonicalMPS(_tensors, _form, _plugs, Ref{Union{Nothing,Tangles.UnsafeScope}}(unsafe))
end

# UnsafeScopeable implementation
ImplementorTrait(::Tangles.UnsafeScopeable, ::MixedCanonicalMPS) = Implements()

Tangles.get_unsafe_scope(tn::MixedCanonicalMPS) = tn.unsafe[]
Tangles.set_unsafe_scope!(tn::MixedCanonicalMPS, uc) = tn.unsafe[] = uc

# TensorNetwork interface
ImplementorTrait(::TensorNetwork, ::MixedCanonicalMPS) = Implements()

Tangles.all_tensors(tn::MixedCanonicalMPS) = collect(tn.tensors)
Tangles.all_tensors_iter(tn::MixedCanonicalMPS) = tn.tensors

Tangles.tensor_at(tn::MixedCanonicalMPS, site::CartesianSite{1}) = tn.tensors[site.id[1]]
Tangles.ind_at(tn::MixedCanonicalMPS, plug::Plug) = tn.plugs[plug]

function Tangles.ind_at(tn::MixedCanonicalMPS, bond::Bond)
    @argcheck hasbond(tn, bond) "Bond $bond not found"
    inds(tensor_at(tn, sites(bond)[1])) ∩ inds(tensor_at(tn, sites(bond)[2])) |> only
end

Tangles.addtensor!(::MixedCanonicalMPS, args...) = error("MixedCanonicalMPS doesn't allow `addtensor!`")
Tangles.rmtensor!(::MixedCanonicalMPS, args...) = error("MixedCanonicalMPS doesn't allow `rmtensor!`")

function Tangles.replace_tensor!(tn::MixedCanonicalMPS, old, new)
    old === new && return tn
    @argcheck issetequal(inds(new), inds(old)) "New tensor must have the same indices as the old tensor"
    for ind in inds(old)
        if !Tangles.isscoped(tn) && size(new, ind) != size(old, ind)
...
function Tangles.replace_ind!(tn::MixedCanonicalMPS, old, new)
    # replace tensors
...
function Tangles.replace_ind!(tn::MixedCanonicalMPS, old_new::AbstractDict)
    # replace tensors
    for (i, tensor) in enumerate(tn.tensors)
...
# Lattice interface
ImplementorTrait(::Tangles.Lattice, ::MixedCanonicalMPS) = Implements()

Tangles.all_sites(tn::MixedCanonicalMPS) = CartesianSite.(1:length(tn.tensors))
Tangles.all_bonds(tn::MixedCanonicalMPS) = [Bond(CartesianSite.((i, i + 1))...) for i in 1:(length(tn.tensors) - 1)]

function Tangles.site_at(tn::MixedCanonicalMPS, tensor::Tensor)
...
function Tangles.bond_at(tn::MixedCanonicalMPS, ind::Index)
...
Tangles.setsite!(::MixedCanonicalMPS, args...) = error("MixedCanonicalMPS doesn't allow `setsite!`")
Tangles.setbond!(::MixedCanonicalMPS, args...) = error("MixedCanonicalMPS doesn't allow `setbond!`")
Tangles.unsetsite!(::MixedCanonicalMPS, site) = error("MixedCanonicalMPS doesn't allow `unsetsite!`")
Tangles.unsetbond!(::MixedCanonicalMPS, bond) = error("MixedCanonicalMPS doesn't allow `unsetbond!`")

# Pluggable interface
ImplementorTrait(::Tangles.Pluggable, ::MixedCanonicalMPS) = Implements()

Tangles.all_plugs(tn::MixedCanonicalMPS) = collect(keys(tn.plugs))
Tangles.all_plugs_iter(tn::MixedCanonicalMPS) = keys(tn.plugs)
Tangles.hasplug(tn::MixedCanonicalMPS, plug) = haskey(tn.plugs, plug)
Tangles.nplugs(tn::MixedCanonicalMPS) = length(tn.plugs)

Tangles.plug_at(tn::MixedCanonicalMPS, ind::Index) = tn.plugs(ind)

Tangles.setplug!(::MixedCanonicalMPS, args...) = error("MixedCanonicalMPS doesn't allow `setplug!`")
Tangles.unsetplug!(::MixedCanonicalMPS, args...) = error("MixedCanonicalMPS doesn't allow `unsetplug!`")
